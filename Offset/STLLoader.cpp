...
// tmp_point *point0, *point1; 比較対象の点．
{
	...
	for (i = 0; i < num_triangles; i++) {
		for (j = 0; j < 3; j++) {
			...
			while (ed != NULL) {
				...
				// 既に登録済み．
				if (((ed->start == triangle[i][j]) && (ed->end == triangle[i][(j + 1) % 3]))
					|| ((ed->start == triangle[i][(j + 1) % 3]) && (ed->end == triangle[i][j])))
					goto try_next;
				ed = ed->next;
			}
